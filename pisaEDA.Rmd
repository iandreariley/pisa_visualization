...
df$wealth.bucket <- cut(df$wealth, breaks = seq(-6.5, 3, .5))
...
ggplot(aes(x = round(wealth/.5) * .5, y = math_score, color = country),
       data = top_and_bottom) +
  geom_line(stat = 'summary', fun.y = median)
```
```{r, echo=FALSE}
ggplot(aes(x = wealth, y = math_score, color = gender),
       data = df) +
  geom_smooth() +
  facet_wrap(~country)
```
